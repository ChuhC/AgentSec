...
| 能力 | 说明 |
|------|------|
| 攻击路径图谱推理 | MVP 不做多步攻击链自动推理；**单 Agent 态势关系图**见 Agent 工作台「态势图谱」Tab |
...
顶栏 Tab：**概览 | 权限管理 | 资产管理 | 态势图谱 | 威胁管理 | 漏洞管理**

| Tab / 交互 | 内容 |
|------------|------|
| 概览 | 四宫格均衡展示；雷达图右下角极小图标（或点雷达）打开权限弹窗；风险摘要可分别跳转威胁 / 漏洞 Tab |
| 资产管理 | 子 Tab：MCP / Skills / Hooks / 知识库 / 通道 / 依赖；列表 + 右侧详情 + 更新 / 禁用 / 卸载 |
| **态势图谱** | 2D 关系图：中心 Agent，辐射 MCP/Skills/知识库/通道/依赖及暴露面、CVE 节点；筛选（全部/仅风险/已启用/类型）、搜索、右侧节点详情；可跳转资产管理 / 威胁 / 漏洞 Tab |
| **威胁管理** | 该 Agent 的暴露面与安全规则事件（`exposure_findings`）；左列表 + 右详情（同 Step 4a 交互，范围限定当前 Agent） |
| **漏洞管理** | 该 Agent 的组件 CVE（`cve_findings`）；全宽表格列表（组件名、风险、CVE 数、版本、操作） |
