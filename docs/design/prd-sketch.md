...
文件：`ui-flow/desktop-macos/step3-results-cards.png`

页面目标：在一屏内告诉用户本次扫描的主要结果和下一步入口。

| 区域 | 内容 |
|------|------|
| 顶栏 | 扫描完成、耗时、路径、重新扫描 |
| 暴露面与基线卡 | 高危 / 中危 / 低危弱化 + 查看详情 |
| 组件漏洞卡 | 高危 CVE / 中危 CVE / 受影响组件数 + 查看详情 |
| Agent 资产卡 | Agent / MCP / Skills 计数 + 查看详情 |
| 优先处理 Top3 | 最多 3 条最高优先级项，辅助用户决定先看哪里 |

原则：

- 不把暴露面风险和组件 CVE 合并成一组高 / 中 / 低。
- 不铺完整列表；完整列表进入详情页。
...
| 区域 | 内容 |
|------|------|
| 左侧列表 | 问题列表，每屏 7–10 行，显示严重度和标题 |
| 右侧详情 | 选中问题的影响、证据、受影响 Agent、建议操作 |
...
- `step7-agent-mcp-tab.png` — MCP Tab（左列表 + 右详情，示例）

页面目标：在**单个 Agent 内**完成概览、组件查看与管理，无需返回首页。

| Tab / 交互 | 内容 |
|------------|------|
| 概览 | 四宫格均衡展示；雷达图右下角极小图标（或点雷达）打开权限弹窗，不占额外版面 |
| 权限详情弹窗 | 聚合 Agent / MCP / Skill / 知识库权限卡片；可筛选；**定位来源** 切 Tab（非固定跳 MCP） |
| MCP / Skills / 知识库 / 依赖 | 列表 + 右侧详情 + 更新 / 禁用 / 卸载 |
