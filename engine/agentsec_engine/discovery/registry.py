...
def discover_all(
    ...
    on_progress: Optional[Callable[[int, int, List[Agent], List[Asset]], None]] = None,
    ...
    for index, cls in enumerate(ADAPTERS):
        ...
        if on_progress:
            on_progress(index + 1, total_adapters, agents, assets)
    if check_updates and not (should_cancel and should_cancel()):
